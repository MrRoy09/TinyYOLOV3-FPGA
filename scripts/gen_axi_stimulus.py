...
Uses the 8x8 stimulus from scripts/stimulus/ (generated by gen_layer0_stimulus.py
with M=0xC0 from quantized_params.npz - hardware_sim.py golden model).

Converts to AXI VIP format:
...
# Use stimulus from gen_layer0_stimulus.py (8x8 image, M=0xC0 from NPZ)
...
def convert_biases_32_to_128(input_file, output_file):
    ...
    with open(input_file, 'r') as fin:
        for line in fin:
            line = line.strip()
            if line:
                # 32-bit value (may be signed, stored as unsigned hex)
                val = int(line, 16)
                biases.append(val)
...
def main():
    ...
    print("Using 8x8 stimulus with M=0xC0 (hardware_sim.py golden)")
    print("=" * 60)
    print()
    ...
    for f in [weights_src, biases_src, pixels_src, expected_src]:
        if not os.path.exists(f):
            ...
            print("Run gen_layer0_stimulus.py first!")
            return
    ...
    convert_weights_72_to_128(weights_src, os.path.join(OUT_DIR, "weights_axi.hex"))
    print()

    # Convert biases (32-bit -> 128-bit packed)
    convert_biases_32_to_128(biases_src, os.path.join(OUT_DIR, "biases_axi.hex"))
    print()
    ...
    print(f"  {OUT_DIR}")
    print()
    print("Next: Run AXI testbench with ./scripts/run_axi_tb.sh")
    print("=" * 60)

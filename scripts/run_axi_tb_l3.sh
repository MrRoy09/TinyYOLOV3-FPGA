#!/bin/bash
# Run the TinyYOLOV3_HW_Complete AXI testbench with Layer 3 (ci_groups=8)
# Usage: ./run_axi_tb_l3.sh
...
echo "========================================"
echo "TinyYOLOV3 AXI Testbench - Layer 3 (ci_groups=8)"
echo "========================================"

# Step 1: Generate Layer 3 stimulus
echo "Step 1: Generating Layer 3 stimulus..."
python3 "$SCRIPT_DIR/gen_axi_stimulus_l3.py"
...
TCL_SCRIPT=$(mktemp /tmp/run_axi_sim_l3_XXXXXX.tcl)
...
# Launch simulation
puts "Launching behavioral simulation for Layer 3 (ci_groups=8)..."
launch_simulation -mode behavioral

# Run longer for Layer 3 (more pixels to process)
puts "Running simulation (timeout: 5ms)..."
run 5ms

puts ""
puts "========================================"
puts "Layer 3 (ci_groups=8) Simulation complete"
puts "========================================"
...
cd "$PROJECT_DIR"
vivado -mode batch -source "$TCL_SCRIPT" -nojournal -nolog 2>&1 | tee /tmp/axi_tb_l3_output.log | tail -200

rm -f "$TCL_SCRIPT"

echo ""
echo "========================================"
echo "Full log saved to: /tmp/axi_tb_l3_output.log"
echo "========================================"

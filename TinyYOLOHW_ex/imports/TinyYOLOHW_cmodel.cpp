...
extern "C" void TinyYOLOHW (
    unsigned int img_width,
    unsigned int in_channels,
    unsigned int out_channels,
    unsigned int quant_M,
    unsigned int quant_n,
    bool isMaxpool,
    bool is_1x1,
    unsigned int stride,
    hls::stream< ap_axiu<512,0,0,0> >& axi_in,
    hls::stream< ap_axiu<512,0,0,0> >& axi_out
) {
    ...
    #pragma HLS INTERFACE s_axilite port=quant_M bundle=control
    #pragma HLS INTERFACE s_axilite port=quant_n bundle=control
    #pragma HLS INTERFACE s_axilite port=isMaxpool bundle=control
    #pragma HLS INTERFACE s_axilite port=is_1x1 bundle=control
...
// Modify contents below to match the function of the RTL Kernel
    unsigned int _data_;
 // SD:..s_intf:axi_in.....m_intf:axi_out

  // axi_in plus 1 to axi_out example
  example_stream_plus1<512,512,0,0,0> (axi_in,axi_out,32);

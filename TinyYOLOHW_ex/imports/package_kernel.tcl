...
proc edit_core {core} {
  ::ipx::associate_bus_interfaces -busif "axi_in" -clock "ap_clk" $core
  set axis_bif      [::ipx::get_bus_interfaces -of $core  "axi_in"] 
  ...
  set_property value        1   $bifparam
  set_property value_source constant     $bifparam
  ::ipx::associate_bus_interfaces -busif "axi_out" -clock "ap_clk" $core
  set axis_bif      [::ipx::get_bus_interfaces -of $core  "axi_out"] 
  set bifparam [::ipx::add_bus_parameter -quiet "TDATA_NUM_BYTES" $axis_bif]
  set_property value        64   $bifparam
  set_property value_source constant     $bifparam
  set bifparam [::ipx::add_bus_parameter -quiet "TUSER_WIDTH" $axis_bif]
  ...
  set reg      [::ipx::add_register -quiet "in_channels" $addr_block]
  set_property address_offset 0x018 $reg
  set_property size           [expr {4*8}]   $reg

  set reg      [::ipx::add_register -quiet "out_channels" $addr_block]
  set_property address_offset 0x020 $reg
  set_property size           [expr {4*8}]   $reg

  set reg      [::ipx::add_register -quiet "quant_M" $addr_block]
  set_property address_offset 0x028 $reg
  set_property size           [expr {4*8}]   $reg

  set reg      [::ipx::add_register -quiet "quant_n" $addr_block]
  set_property address_offset 0x030 $reg
  set_property size           [expr {4*8}]   $reg

  set reg      [::ipx::add_register -quiet "isMaxpool" $addr_block]
  set_property address_offset 0x038 $reg
  set_property size           [expr {1*8}]   $reg

  set reg      [::ipx::add_register -quiet "is_1x1" $addr_block]
  set_property address_offset 0x040 $reg
  set_property size           [expr {1*8}]   $reg

  set reg      [::ipx::add_register -quiet "stride" $addr_block]
...
proc package_project {path_to_packaged kernel_vendor kernel_library kernel_name} {
  ...
  foreach user_parameter [list C_S_AXI_CONTROL_ADDR_WIDTH C_S_AXI_CONTROL_DATA_WIDTH C_AXI_IN_TDATA_WIDTH C_AXI_OUT_TDATA_WIDTH] {
